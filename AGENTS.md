...
- **North star:** login-protected workspace for **running the teaching business** (content publish + People/CRM + student LTV / Payment Links + Google Calendar lesson requests today; later communications and financial summaries). Always `/studio` and `/studio/*` only—SWA `authenticated`, personalized to the signed-in user, `noIndex`, excluded from sitemap. Phased backlog: [`docs/plans/studio-teaching-business.md`](docs/plans/studio-teaching-business.md) (`STUDIO-*`). Payments: [`docs/plans/lesson-payments.md`](docs/plans/lesson-payments.md). Calendar connect: [`docs/runbooks/studio-calendar.md`](docs/runbooks/studio-calendar.md).
- **Authentication ≠ authorization.** Entra + SWA Easy Auth only prove identity. Keep enterprise-app **Assignment required** off (`require_app_role_assignment = false`) so any tenant member or guest can sign in (Assignment required causes `AADSTS50105`). Never treat `authenticated` or a valid `x-ms-client-principal` as permission to publish, manage People, connect Calendar, or change access. Enforce on **every** privileged API call with `permissionGate()` against the catalog in `api/src/lib/permissions.js` (`content.publish`, `people.read` / `people.write`, `calendar.read` / `calendar.write` / `calendar.connect`, `users.manage`). User profiles in Table Storage are the SoT; `ALLOWED_USER_IDS` only bootstraps a missing Super Administrator profile on first session. Public exceptions (`contactInquiry`, `lessonPayConfig`, `stripeWebhook`, `calendarWatch`, `lessonAction`) use Turnstile / sanitized config / Stripe signatures / channel or signed tokens — not Studio login. See [`docs/architecture/authentication-authorization.md`](docs/architecture/authentication-authorization.md), [`.cursor/rules/studio-auth.mdc`](.cursor/rules/studio-auth.mdc), and [manage-access.md](docs/runbooks/manage-access.md).
- User guide: [`/studio/help`](src/pages/studio/help/index.astro) — Help icon on each Studio screen (not a workflow tab). Document only shipped capabilities. Home: [`/studio`](src/pages/studio/index.astro). Publish: [`/studio/content`](src/pages/studio/content.astro). People: [`/studio/people`](src/pages/studio/people.astro) (`STUDIO-P1-*`). Payment status: [`/studio/students/payments`](src/pages/studio/students/payments.astro) (`STUDIO-P2-*`). Schedules: [`/studio/calendar`](src/pages/studio/calendar.astro) (`STUDIO-P3-*`). Calendar connect: [`/studio/admin/calendar`](src/pages/studio/admin/calendar.astro). Access: [`/studio/admin/access`](src/pages/studio/admin/access.astro). Read-only canary: [`/studio/health`](src/pages/studio/health.astro) (`TEST-C-005`).
...
- Signed-in users may open Studio, help, and `/studio/health` even without catalog permissions. People requires `people.read` / `people.write`; schedules require `calendar.read` / `calendar.write`; Calendar connect requires `calendar.connect`; publish requires `content.publish`; `/studio/admin/access` requires `users.manage`. There is no Gemini tool that writes People or lessons.
