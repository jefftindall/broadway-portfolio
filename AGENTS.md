...
- Signed-in users may open Studio, help, and `/studio/health` even without catalog permissions. People requires `people.read` / `people.write`; publish requires `content.publish`; `/studio/access` requires `users.manage`. There is no Gemini tool that writes People.
