...
- **SWA cache:** HTML `max-age=30`; hashed `/_astro/*` and `/images/_derived/{sha}/…` are immutable long-cache (new build → new URL). Original `/images/*` is 7 days. `/studio`, `/studio/*`, and `/api/*` are `private, no-store` so login 302s are not replayed. Everyday Studio/markdown deploys are not pinned by the long-cache assets. Flush/diagnose: [`docs/runbooks/swa-caching.md`](docs/runbooks/swa-caching.md).
