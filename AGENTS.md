...
**Only bootstrap → environments.** `infra/bootstrap` creates shared resources (`kv-elyse-shared`, ACS, OIDC, budget, Stripe catalog). Staging/prod **consume** them by well-known name (`data.azurerm_key_vault.shared`). That is the allowed dependency.
...
- **Lesson payments:** Stripe Payment Links, gated by SWA `LESSON_PAYMENTS_ENABLED` (staging **true**, prod **false** until go-live). CD’s single artifact cannot bake different `PUBLIC_*` values per env — `/lessons/book` reads `GET /api/lessonPayConfig`. Test/live keys, webhook secrets, and Payment Links live in **`kv-elyse-shared`**. Bootstrap Terraform owns products, prices (USD cents from [`lessons-book.md`](src/content/pages/lessons-book.md)), and webhook endpoints. See [`docs/plans/lesson-payments.md`](docs/plans/lesson-payments.md) and [rotate-secrets.md](docs/runbooks/rotate-secrets.md).
