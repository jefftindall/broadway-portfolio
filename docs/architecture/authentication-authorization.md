...
| `POST /api/lessonBook` | Same gate; `contactId` from session or `bookForContactId` (linked student); rate-limited; uses `createLessonsWithCalendar` |
| `GET /api/accountLessons` | `contactGate()` + linked contact; own lessons + linked students when parent |
