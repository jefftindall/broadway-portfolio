...
|--------------|--------|----------------|
| Phase 0 — Plan, scorecard, `ALERT-*` naming | `done` | Monthly run habit / first scheduled confirm under P0-004 / P5 |
| Phase 1 — KV-backed email + SMS + voice | `done` | — |
...
| Phase 6 — Deploy-quiet + API error-budget Sev2 | `done` | Residual: confirm next prod apply + content publish does not page `alert-elyse-failed-requests-prod` |
...
4. **Output:** Commit and push scorecard files directly to `main` as `elyse-portfolio-studio[bot]` (App is a **Protect main** bypass actor — same as Studio publishes). Push via [`scripts/git-push-main-rebase.sh`](../../scripts/git-push-main-rebase.sh) (fetch + `rebase --autostash` onto latest `main`, retry tip races; no force-push) so concurrent merges during the Azure/GA probe window do not fail the job, and incidental working-tree dirt (e.g. `chmod +x` on scripts) cannot block the rebase. Keep workflow shell scripts `100755` in git. Do **not** open a PR. Do **not** push with the job `GITHUB_TOKEN` (`github-actions[bot]`). Do **not** trust `GET /repos` `.permissions.push` for installation tokens (often all-false).
...
### Scorecard shape (`sitePerformance` in evaluation JSON)
...
6. **Monthly workflow wiring** (`done` in repo)  
   After Azure login / existing KV fetch pattern: load the two secrets into env or `0600` temp files for `ops-scorecard-refresh.mjs` via `scripts/fetch-ga-scorecard-secrets.sh`. Call Data API `runReport` for the previous month: metrics `sessions` + `activeUsers`; dimension `pagePath` (or `unifiedPagePathScreen`) limited to top 5–10; filter out `/studio` if any leaks. On auth/API failure → set `sitePerformance.status = "stale"` and continue (same soft-fail pattern as optional SLIs) — do **not** fail the whole scorecard job solely because GA is down. **Operator residual:** replace `REPLACE_ME` placeholders with real KV values so visits/top pages leave `stale`.
...
Exact Kusto for contacts/updates lives in [observability.md](../runbooks/observability.md) (`OPS-P5-004`).
...
| **SLO-7** | API request success | **99.9% over 7 days** | Wall-clock minutes with **2+** 5xx/timeout (exclude 4xx + CD quiet window) / 10080 | Measurable (`OPS-P6-002`) |

**Error budget (homepage/materials):** 99.8% / 7d ≈ **20 minutes** / week (~≤2 failed 10-minute probes). Page on the first Sev1 window; use the weekly % as the scorecard input for `slo` / `alerting`.

**Error budget (API, SLO-7):** 99.9% / 7d ≈ **10 minutes** of bad API time / week. A minute is bad only with 2+ server errors, so a single 502 does not consume budget. Sev2 pages when the **2d** burn exceeds the proportional budget (~2.9 min → alert at 3+ bad minutes). Monthly scorecard scores the full 7d window.
...
3. Homepage availability → **critical**; API error-budget burn (SLO-7 99.9%) → **notify**.
...
| `OPS-P0-004` | Monthly GitHub Actions workflow to re-evaluate scorecard + commit to `main` | `schedule` + `workflow_dispatch`; updates scorecard doc; Studio GitHub App push to `main`; CD `paths-ignore` for scorecard-only commits; **on job failure**, ACS email to `ALERT-EMAIL` only ([`ops-scorecard-failure-email.mjs`](../../scripts/ops-scorecard-failure-email.mjs)) | `done` |
...
| `OPS-P4-002` | Monthly scorecard ACS digest + Cost Management spend/MoM | Digests to `ALERT-EMAIL` + `SITE-CONTACT-EMAIL`; body scores + USD + site activity; spend in scorecard `costProbe`; digest format SoT [ops-monthly-checkin-email.mdc](../../.cursor/rules/ops-monthly-checkin-email.mdc) | `done` |
...
<details>
<summary><code>OPS-P5-*</code> — Phase 5 acceptance</summary>

- [x] `GA-PROPERTY-ID` / `GA-DATA-API-SA-JSON` placeholders in bootstrap shared KV + [ga-data-api-access.md](../runbooks/ga-data-api-access.md)
- [x] `scripts/fetch-ga-scorecard-secrets.sh` + monthly workflow fetch (line-masked; temp SA file cleaned up)
- [x] App Insights calendar-month contacts (casting/lesson) + Studio publishes in `sitePerformance`
- [x] GA4 visits + top pages soft-fail to `stale` when creds missing
...
| Action ID | Work | Acceptance criteria | Status |
|-----------|------|---------------------|--------|
| `OPS-P6-001` | Stop Sev2 paging on SWA Function recycle during CD | Failed-request alert is 5xx-only, ignores 15m before / 10m after `DeployStarted`/`DeployCompleted`; CD emits `DeployStarted` before upload; always keep `api_location` (omitting it unlinks managed Functions); cost table includes system-log alert meters | `done` |
| `OPS-P6-002` | Sev2 uses 99.9%/7d API error budget (not single errors) | SLO-7 committed; a minute is bad only with 2+ 5xx/timeouts; 2d alert fires at 3+ bad minutes; scorecard probes 7d; constants in [`api-error-budget.mjs`](../../scripts/lib/api-error-budget.mjs) | `done` |

<details>
<summary><code>OPS-P6-001</code> — Phase 6 acceptance</summary>

- [x] Replace count&gt;0 `requests/failed` metric alert with scheduled query (5xx / resultCode 0, deploy quiet window)
...
<details>
<summary><code>OPS-P6-002</code> — API error-budget Sev2</summary>

- [x] SLO-7: API request success **99.9% / 7d** (~10 min bad-time budget)
- [x] Bad minute = 2+ server errors (5xx / timeout); 4xx and **every** CD quiet window excluded (not only the latest deploy)
- [x] Sev2 scheduled query: 2d window, fire when BadMinutes &gt; 2 (3+ min vs ~2.9 min proportional budget)
- [x] Monthly scorecard Kusto probe + `committedSlos` row (stale until next `--azure` run)
- [x] Shared constants + unit tests in [`scripts/lib/api-error-budget.mjs`](../../scripts/lib/api-error-budget.mjs)
...
OPS-P0-001 (this plan / AI guidance) [done]
    ...
    │                               ├── OPS-P5-002 (GA Data API + KV) [done]
    ...
    └── OPS-P4-001 (subscription budget = ceil(expected×1.25); 80% alert) [done]
            └── OPS-P6-001 (deploy-quiet failed-request Sev2) [done]
                    └── OPS-P6-002 (API error-budget 99.9%/7d) [done]
