...
- Browser sampling: **25%** staging, **10%** prod (`PUBLIC_APPINSIGHTS_SAMPLE_PERCENT`)
- Server: adaptive sampling in `api/host.json` with dependency tracking on
- Availability test: **prod only**, homepage every **10 minutes**, one geo (`us-va-ash-azr`)

To temporarily raise browser sampling, set `PUBLIC_APPINSIGHTS_SAMPLE_PERCENT` in the workflow (or rebuild with a higher env value) and redeploy.
...
| `StudioPublishRequested` | `api` updateContent |
...
| `StudioPublishUiSuccess` / `StudioPublishUiFailed` | Studio UI |
...
```kusto
customEvents
| where name in ("StudioPublishRequested", "StudioToolExecuted", "GitHubCommitSucceeded", "GitHubCommitFailed", "StudioPublishUiSuccess", "StudioPublishUiFailed", "DeployCompleted")
| order by timestamp desc
| take 100
```
