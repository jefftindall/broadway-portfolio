...
**If API calls still 403 after Full:** confirm the grant is on the same property as `GSC-SITE-URL` (domain `sc-domain:…` vs URL-prefix `https://…/`), wait a minute for propagation, and re-run **Search: monthly signals**.
