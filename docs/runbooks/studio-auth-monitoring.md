...
| Interactive login fails `AADSTS50105` | Enterprise-app **Assignment required** is on. User login must stay open (`require_app_role_assignment = false`). Re-apply the env stack; do not assign the tester in Users and groups as a workaround. Authorization stays on the permission catalog. |
