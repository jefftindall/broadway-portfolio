...
**Current-values rule:** Discrete updates (rates, reel, short bio, performer facts) must ground tool args in live catalog lines (`Lesson rates (live)`, `Reel URL (live)`, `Short bio (live)`, `Performer facts (live)`). Never blank a field or invent a parallel value when the catalog already shows the current one. Studio UI prefills the same live values in hub editors and structured Preview.
