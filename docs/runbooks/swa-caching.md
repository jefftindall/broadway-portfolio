...
| `/studio`, `/studio/*`, `/api/*`, `/api/contactInquiry`, `/api/lessonPayConfig`, `/api/stripeWebhook`, unmatched / 401 login 302s | `private, no-store` | Auth-gated, inquiry, lesson-pay config, and Stripe webhook responses. A cached 302 to `/.auth/login/aad` replays after Entra returns to `/studio` and loops. 401 overrides inherit `globalHeaders`, not the `/studio` route. |
